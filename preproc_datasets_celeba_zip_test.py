...
def custom_reshape(img, mode='bicubic', ratio=0.99):   # more to be implemented here
    ...
    if full_size < 224:
        ...
        m = torch.nn.ConstantPad2d(
            (pad_1, 224-full_size-pad_1, pad_2, 224-full_size-pad_2), 1.)
        reshaped_img = m(img)
    else:
...
def open_image_folder(source_dir, *, max_images: Optional[int]):
    ...
    input_images = [str(f) for f in sorted(Path(op.join(source_dir, 'images')).rglob('*'))
                    if is_image_ext(f) and os.path.isfile(f) and op.basename(f).split('.')[0] in data_list]
    ...
    def iterate_images():
        for idx, fname in enumerate(input_images):
            img_path = fname
            txt_path = op.join(
                source_dir, f"celeba-caption/{op.basename(img_path).split('.')[0]}.txt")
            # print(txt_path)
...
def open_image_zip(source_dir, *, max_images: Optional[int]):
    ...
    def iterate_images():
        with zipfile.ZipFile(source_dir, mode='r') as z:
            for idx, fname in enumerate(input_images):
                txt_path = fname.replace(
                    "images", "celeba-caption").replace("jpg", "txt")
...
def make_transform(
    ...
    resample = {'box': PIL.Image.BOX,
                'lanczos': PIL.Image.LANCZOS}[resize_filter]
    ...
    if transform == 'center-crop':
        if (output_width is None) or (output_height is None):
            error('must specify --width and --height when using ' +
                  transform + 'transform')
        ...
        try:
            ...
            if dataset_attrs == cur_image_attrs:
                ...
                with torch.no_grad():
                        ...
                        if text_line != '' and not text_line.isspace():
                            try:
                                tokenized_text = clip.tokenize(
                                    [text_line]).to(device)
                                text_feature = clip_model.encode_text(
                                    tokenized_text)
                    ...
                    clip_txt_features.append(
                        [archive_fname, text_feature_list])
